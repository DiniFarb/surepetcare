...
class PetCare extends EventEmitter {
    ...
    setDoorState(name = String, command = Number) {
        try {
            ...
            if(!this.utils.doorStates[command]) throw `command: ${command} can't be used for doors`;
            let { id } = this.household.petCareData.devices.find(device => device.name === name);
            if(!id) throw `No door with name ${name} found`;
    ...
    setPetPlace(name = String, command = Number) {
        ...
        try {
            if(!this.utils.placeNames[command]) throw `command: ${command} can't be used for pet placeing`; 
            let pet = this.household.petCareData.pets.find(pet => pet.name === name);
            if(!pet) throw `No pet with name ${name} found`;
            ...
            log.error(`set pet place faield: ${err}`);
            this.emit('error', `set pet place faield: ${err}`);
        };
    };
    ...
    resetFeeders(command = Number) {
        ...
        try {
            if(!this.utils.tareText[command]) throw `command: ${command} can't be used for feeders`; 
            ...
            if(devices.length == 0) throw `No feeders found`;
                ...
                sleep().then(()=>{
                    ...
                    .then(res => {
                        if (res.results){
                            ...
                            this.emit('info', `reseted feeder: ${devices[i].name}`);
                        } else {
    ...
    resetFeeder(name = String, command = Number) {
        ...
        try {
            let { id } = this.household.petCareData.devices.find(device => device.name === name);
            if(!id) throw `No ${name} device found`;
            if(!this.utils.tareText[command]) throw `command: ${command} can't be used for feeders`; 
            this.#petCareAPI.resetFeeder(command, id).then(res => {
                    if (res.results){
                        ...
                        throw `result of feeder ${name} was undefined`
                    }
            }).catch(err => {
                throw err;
            });
