        ...
        this.petCareData.pets.forEach($pet => {
            if ($pet.status.feeding) {
                ...
                this.pets[$pet.name] = {
                    ...
                    currentLeft: Math.round(this.#getNewestEntry(eating).weights[0].frames[0].current_weight),
                    currentRight: Math.round(this.#getNewestEntry(eating).weights[0].frames[1].current_weight),
                    lastEatenLeft: Math.round(this.#getNewestEntry(eating).weights[0].frames[0].change * -1) ,
                    lastEatenRight: Math.round(this.#getNewestEntry(eating).weights[0].frames[1].change * -1) ,
                    ...
                    lastFillLeft: Math.round(this.#getNewestEntry(foodFilling).weights[0].frames[0].current_weight),
                    lastFillRight: Math.round(this.#getNewestEntry(foodFilling).weights[0].frames[1].current_weight),
                ...
                this.pets[$pet.name].eatenDry = this.pets[$pet.name].lastFillLeft - this.pets[$pet.name].currentLeft;
                this.pets[$pet.name].eatenWet = this.pets[$pet.name].lastFillRight - this.pets[$pet.name].currentRight;
                ...
                if (entry.type === 22) {
                    Object.keys(this.pets).forEach(petName => {
                        if (entry.devices[0].name === this.pets[petName].deviceName) {
                            ...
                            updates.readyMsg.push(this.utils.petHasEatonText(this.pets[petName].name, eatenDry,eatenWet));
                        }
                    });
                };
                //Reset Feeder
                if (entry.type === 24) {
                    Object.keys(this.pets).forEach(petName => {
                        if (entry.devices[0].name === this.pets[petName].deviceName) {
                            ...
                            updates.readyMsg.push(this.utils.resetFeederText(entry.devices[0].name,JSON.parse(entry.data).tare_type));
                        }
                    });
                };
                //Battery threshold
