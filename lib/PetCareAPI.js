...
class PetCareAPI {
  ...
  async resetFeeder(tareNumber, device_id) {
    try {
      const response = await fetch(`${baseURL}/device/${device_id}/control`, {
        headers: {
          accept: "application/json, text/plain, */*",
          authorization: `Bearer ${this.#token}`,
          "content-type": "application/json",
      ...
      throw `Petcare | reset feeder with ID ${device_id} failed: ${err}`;
    }
  }
